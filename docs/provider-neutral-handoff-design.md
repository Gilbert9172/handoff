...
> 상태: Historical design record — 현재 동작은 아래 현행 기준과 skills/*/SKILL.md 우선
> 작성일: 2026-08-27
...
이 문서는 설계 과정과 당시 실측의 기록이다. 아래 과거 절의 미구현 계획이나 번복 전 설명을 현재 계약으로 해석하지 않는다. 실행 지침은 `skills/*/SKILL.md`, 사용 안내는 README를 기준으로 한다.

- 정체성: 플랫폼이 다른 로컬 에이전트 사이의 작업 인계와 장기 작업 맥락 보존.
- 여섯 섹션을 유지하며 사용자 제약, 작업 위치, 결정 근거, 검증 여부, 막힌 이유와 재개 조건을 필요한 곳에 기록한다.
- 완료는 Goal과 결과 근거로 판단한다. Next Steps가 비었다는 사실만으로 완료를 선언하지 않는다.
- 서로 다른 발견은 누적하고 중복은 병합한다. 정정된 결론은 대체 관계와 이유를 보존한다. 보수적 compact와 200줄 알림을 유지하며 강제 줄 수 상한은 없다.
- 관련 후속 문서는 선행 문서 경로와 이어받을 결정·제약·새 범위를 기록한다.
- finish는 Next Steps를 완료·미완료·폐기로 구분하고 미확인 결과도 표시한다. 사용자 종료 결정을 실제 검증 근거로 바꾸지 않는다.
- 공통 본문은 현재 호스트의 질문 수단을 사용하고 기존 답변·승인을 존중한다. 스킬 위치로 `HANDOFF_PLUGIN_ROOT`를 설정한다. 호스트별 hook 설정은 유지한다.
- 저장 위치는 같은 홈·같은 프로젝트 절대경로 기준이다. 원격 동기화, `HANDOFF_HOME`, 문서 frontmatter 자동 부착, 스크립트 기반 작업 slug 정규화는 현재 구현에 없다.
- Codex UI 메타데이터의 기존 호출 정책은 유지한다. 아래 버전별 호환성 관찰은 현재 런타임 전체에 대한 보장이 아니다.
...
**압축 (`--compact`, 결정 2026-09-02).** 누적은 handoff를 유용하게 만드는 동시에 무한히 키운다. 압축은 문서를 **끝내지 않고** 줄인다 — 작업이 계속되므로 다음에 읽을 에이전트에게 여전히 쓸모 있어야 한다.

- 오래된 What Worked / What Didn't Work 항목만 한 줄로 묶는다. 최근 항목은 살아 있는 맥락이므로 그대로 둔다.
- 실패한 접근을 통째로 지우지 않는다. 지우면 같은 실수를 반복하게 되고, 그게 이 섹션의 존재 이유다.
- Goal · Current Progress · Next Steps는 건드리지 않는다. 이미 최신 상태다.

압축과 봉인은 개념적으로 별개다 — 압축은 문서를 줄이는 것이고 봉인은 흐름을 끝내는 것이다. 압축이 필요한 시점은 대개 작업이 **진행 중**일 때이므로 `save`가 기본 창구를 맡는다.

**개정 (2026-09-03):** `finish`도 봉인 직전에 한 번, 문서가 200줄을 넘으면 같은 규칙을 자동으로 적용한다 — 봉인된 문서는 다시 손댈 수 없으므로 그게 마지막 기회이기 때문이다. 사용자 확인 없이 자동으로 도는 점만 `save --compact`와 다르다. 자세한 내용은 §6.5.

### 5.5 수명 경계 (결정, 2026-09-02)

handoff에는 시작(`save`가 파일을 만든다)은 있었지만 **끝이 없었다.** 그 결과 두 문제가 같은 뿌리에서 나왔다.

- 완료를 표현할 수단이 `delete`(파일 삭제)뿐이었다.
- §5.4의 누적 규칙 때문에 같은 slug에 계속 덧붙어 문서가 단조 증가했다. resume은 문서를 전부 읽으므로, **컨텍스트를 아끼려는 도구가 스스로 컨텍스트를 잠식하는** 구조였다.

`finish`가 그 경계를 긋는다.

- **handoff의 단위는 프로젝트가 아니라 "이어서 할 하나의 작업 흐름"이다.** Goal이 더는 지금 하는 일을 설명하지 못하면 그건 다른 작업이고, 새 문서를 만든다.
- **봉인은 논리 플래그가 아니라 물리적 사실이다.** 봉인된 문서는 `$dir/done/`으로 옮긴다. `scan`의 glob이 하위 디렉토리를 보지 않으므로 `list`·`resume`이 실수로 집어 올릴 여지가 구조적으로 없다. 파일 안의 `**Status**:` 줄은 종료 사유를 사람이 읽기 위한 기록이며 필터의 근거가 아니다.
- **봉인된 문서에는 덧붙이지 않는다.** 이것이 문서 비대화를 실제로 멈추는 규칙이다. 흐름이 끝나면 다음 작업은 자연히 새 파일이 된다.
- **종료 사유를 구분한다** — `done`(목표 달성)과 `abandoned`(목표 폐기). 후자는 사유 한 줄을 함께 남긴다. 남기지 않으면 보관된 문서가 "왜 멈췄는지"를 다시 추적하게 만들어, handoff의 존재 이유와 정면으로 어긋난다.

**완료 조건(Definition of Done)은 문서에 넣지 않는다.** 검토했으나 기각했다 — §5.1의 모든 섹션은 "다음 세션이 이어받는다"는 하나의 목적에 봉사하는데, 완료 조건은 이어받는 데 필요한 정보가 아니라 종료를 판정하는 정보다. 목적이 다른 데이터를 인계 문서에 얹는 셈이고, 이슈 트래커의 개념을 끌어오는 것이다. 종료 판정은 §6.5대로 사용자가 한다.
...
### 6.5 `finish` (결정, 2026-09-02)

작업이 끝난 handoff를 봉인한다 (§5.5).

1. **(개정, 2026-09-03)** 보여주기 전에 Current Progress를 이번 세션 내용으로 갱신하고, What Worked/What Didn't Work에 새 발견이 있으면 §5.4 규칙대로 누적한다. Next Steps는 건드리지 않는다 — 봉인 이후엔 그 필드를 읽고 움직일 "다음 세션"이 없으므로, 이 시점부터는 계획이 아니라 "어디서 멈췄는지"를 보여주는 기록으로 남기 때문이다. 문서가 200줄을 넘으면 §5.4의 압축 규칙을 자동으로 적용한다.
2. Goal · Current Progress · 남은 Next Steps를 보여준다.
3. 종료 사유를 받는다 — `done` 또는 `abandoned`(사유 한 줄). **(개정, 2026-09-03)** 남은 Next Steps가 있는데 `done`을 고르면 한 번 더 확인한다 — 거부가 아니라 재확인이며, 사용자가 `done`을 유지하면 그대로 진행한다.
4. 문서 최상단에 `**Status**: <사유> (YYYY-MM-DD)` 줄을 남긴다. 첫 헤딩 앞에 두어 `scan`의 Goal 파싱과 겹치지 않게 한다.
5. `$dir/done/`으로 옮긴다. 이름이 충돌하면 덮어쓰지 않고 사용자에게 묻는다.
6. 보관 경로와, 이후 `list`·`resume`·`save`에서 제외된다는 사실을 보고한다.

**종료 판정의 주체는 사용자다.**

- `finish`는 끝났는지 **판단하지 않는다.** 사용자가 이 skill을 호출한 행위 자체가 종료 선언이고, skill은 봉인 대상을 보여주고 결정을 기록할 뿐이다. **(개정, 2026-09-03)** 기록을 최신 상태로 맞추는 것과, 화면에 보여준 내용이 선택과 어긋날 때 한 번 되묻는 것은 이 원칙의 예외가 아니다 — 둘 다 사용자가 더 나은 정보로 같은 결정을 내리게 돕는 것이지, skill이 대신 결정하는 게 아니다.
- **남은 Next Steps는 봉인을 막지 않는다.** 계획은 낡는다 — 도중에 방향이 바뀌었다면 옛 Next Steps는 미완의 작업이 아니라 폐기된 계획이다. 남은 항목을 알리고 필요하면 한 번 되묻되, 그 자체로 거부의 근거가 되지는 않는다.
- Next Steps가 비었다는 것도 종료의 증거가 아니다. 적어둔 게 없다는 뜻이지 할 일이 없다는 뜻이 아니다.
- 다른 skill은 `finish`를 **알릴 뿐 대신 실행하지 않는다.** `save`는 저장 후 안내 한 줄로, `resume`은 작업이 일단락될 때 경로 안내로 언급한다. 어느 쪽도 질문으로 사용자를 막지 않는다 — `save`는 대개 컨텍스트가 찬 상태에서 세션을 끝내려 할 때 호출되므로, 그 자리에서 질문으로 붙잡으면 흐름을 끊는다. 질문으로 막으면 사용자가 얼결에 승인하게 되어, 배제하려던 "모델의 자동 판단"과 실질적으로 같아진다.

Claude에서는 `disable-model-invocation: true`로 자동 호출을 차단한다. Codex에서는 같은 보장을 얻을 수 없다 (§7.3) — 방어선은 위 확인 절차다.
...
- 모든 skill이 Codex에서 기본값(자동 활성화 허용)을 쓴다. `list`·`delete`·`finish`의 `agents/openai.yaml`은 `interface`(표시 이름) 용도로만 남기고 `policy.allow_implicit_invocation: false`는 넣지 않는다 — 이 필드가 `$` 명시 호출까지 막는다는 게 실측으로 확인됐기 때문이다 (§7.1 "번복" 참고). Codex에서 `delete` 오남용을 막는 건 이 policy가 아니라 §6.4의 확인 절차다. `finish`도 같다 — Claude의 `disable-model-invocation`에 해당하는 보장이 Codex에는 없으므로, "종료 판정은 사용자가 한다"는 §6.5의 원칙은 Codex에서 본문의 확인 절차만으로 지탱된다.
...
- `list`·`delete`·`finish`의 `agents/openai.yaml`에는 `policy.allow_implicit_invocation: false`를 넣지 않는다(§7.1 "번복" 참고 — 이 필드가 `$` 명시 호출까지 막는 것을 실측했다). 따라서 Codex에서 삭제 오남용을 막는 건 frontmatter/policy가 아니라 §6.4의 확인 절차(삭제 전 slug·Goal 표시 후 승인)뿐이라는 점을 항상 전제한다.
