...
압축과 봉인은 별개다. 압축이 필요한 시점은 대개 작업이 **진행 중**일 때이므로, 봉인(`finish`)에 묶어두면 정작 필요한 구간에서 쓸 수 없다. 그래서 압축은 `save`가 담당한다.
...
1. Goal · Current Progress · 남은 Next Steps를 보여준다.
2. 종료 사유를 받는다 — `done` 또는 `abandoned`(사유 한 줄).
3. 문서 최상단에 `**Status**: <사유> (YYYY-MM-DD)` 줄을 남긴다. 첫 헤딩 앞에 두어 `scan`의 Goal 파싱과 겹치지 않게 한다.
4. `$dir/done/`으로 옮긴다. 이름이 충돌하면 덮어쓰지 않고 사용자에게 묻는다.
5. 보관 경로와, 이후 `list`·`resume`·`save`에서 제외된다는 사실을 보고한다.

**종료 판정의 주체는 사용자다.**

- `finish`는 끝났는지 **판단하지 않는다.** 사용자가 이 skill을 호출한 행위 자체가 종료 선언이고, skill은 봉인 대상을 보여주고 결정을 기록할 뿐이다.
- **남은 Next Steps는 봉인을 막지 않는다.** 계획은 낡는다 — 도중에 방향이 바뀌었다면 옛 Next Steps는 미완의 작업이 아니라 폐기된 계획이다. 남은 항목을 알리되 판단하지 않는다.
