...
Claude Code와 Codex의 에이전트 사이에서 **작업을 인계하고, 장기 작업의 맥락을 이어주는** 플러그인입니다.
...
- **작업을 오가도 빠르게 복귀** — 작업마다 노트가 따로 있어 `list`로 무엇이 어디까지 왔는지 한눈에 보고, `resume`이 Goal·What Worked·Next Steps를 요약해줘 **목표와 필요한 맥락을 빠르게 파악하고** 중단했던 지점으로 바로 돌아갑니다.
...
관련 후속 작업을 새 노트로 만들면 **선행 핸드오프의 정확한 경로, 이어받는 결정·제약, 새 작업 범위**를 Current Progress에 남깁니다. 봉인한 원본은 수정하지 않습니다.
...
- 노트를 전부 읽은 뒤 필요한 선행 기록과 실제 작업 상태를 확인하고 **Goal · What Worked · Next Steps를 짧게 요약**합니다. 이미 실행을 요청했다면 이어서 진행하고, 맥락 로드만 요청했다면 실행 여부를 확인합니다. **What Didn't Work**에 적힌 실패 방법은 같은 조건에서 반복하지 않고, **Parked**에 적힌 일은 Goal 밖이므로 하지 않습니다.
- Next Steps가 비어 있어도 완료로 단정하지 않습니다. 기록된 결과가 Goal의 완료 조건을 충족할 때만 `finish`를 안내하고, 그렇지 않으면 미확인 사항이나 대기 조건을 알립니다.
...
- 보여주기 전에 **Current Progress**를 이번 세션 내용으로 먼저 갱신하고, 200줄을 넘으면 `--compact`와 같은 방식으로 자동 정리합니다. Next Steps의 각 항목은 실제 결과에 따라 완료·미완료·폐기로 표시하고, 확인할 수 없는 결과는 미확인으로 남깁니다.
- **Goal · Current Progress · 남은 Next Steps · Parked**를 보여준 뒤, 어떻게 끝났는지 고릅니다. Parked는 일부러 빼둔 일이라 봉인에 걸리지 않고, 봉인 후 새 노트로 시작할 수 있다고 안내합니다 — **done**(목표 달성) 또는 **abandoned**(중단, 사유 한 줄). 미완료·미확인 Next Steps가 남아 있는데 done을 고르면 정말 그런지 한 번 더 확인합니다(거부는 아니고, 확인 후 그대로 진행).
- 문서 맨 위에 `**Status**: done (2026-09-01)` 같은 줄을 남기고 `done/`으로 옮깁니다.
- 봉인 후에는 list·resume에 뜨지 않고, save도 덧붙이지 않습니다. 파일은 그대로 남아 있습니다.

> **끝났는지는 사용자가 정합니다.** Next Steps가 남아 있어도 봉인을 막지 않습니다 — 도중에 방향이 바뀌어 원래 계획이 폐기됐을 수 있으니까요. 남은 항목을 알려주고 한 번 확인은 하되 최종 판단은 사용자 몫이고, 이 커맨드는 **자동으로 실행되지 않습니다.**
...
각 노트는 다음 여섯 섹션으로 구성됩니다. 관련이 있으면 사용자 제약, 실제 작업 위치·파일, 중요한 결정과 이유, 검증 결과와 미검증 사항, 막힌 이유와 재개 조건을 함께 기록합니다. 코딩 작업은 필요한 브랜치·워크트리·미커밋 변경도 포함합니다.
...
**기록된 결과가 Goal의 완료 조건을 충족하고 필요한 작업이 남지 않았을 때** `save`와 `resume`은 `finish`를 한 줄로 권합니다. 목록이 비어 있다는 것만으로 완료를 판단하지 않습니다. Parked 항목은 새 노트로 시작할 수 있습니다. 봉인은 여전히 사용자가 합니다.

### 업데이트 시 병합 규칙 (`/handoff:save`가 자동 적용)

- **Current Progress · Next Steps** → 최신 상태로 **새로 씀**
- **What Worked · What Didn't Work · Parked** → 서로 다른 기록은 **누적**, 같은 사실은 근거와 조건을 보존하며 병합. 바뀐 결론은 대체 관계와 이유를 표시
...
handoff는 저장소가 아니라 홈 디렉토리의 프로젝트별 폴더에 저장됩니다. 자동 공유 범위는 같은 홈 디렉토리와 같은 프로젝트 절대경로를 쓰는 로컬 환경입니다. 다른 기기나 체크아웃으로의 자동 동기화는 제공하지 않습니다. Claude Code와 Codex 어느 쪽에서 저장·조회하든 같은 경로를 씁니다:
...
```sh
sh "${HANDOFF_PLUGIN_ROOT}/scripts/handoffs.sh" dir         # 이 프로젝트의 handoff 디렉토리
sh "${HANDOFF_PLUGIN_ROOT}/scripts/handoffs.sh" dir done    # 봉인된 노트 보관 폴더
sh "${HANDOFF_PLUGIN_ROOT}/scripts/handoffs.sh" scan        # 진행 중 노트별: 슬러그 · 수정일 · 줄 수 · 상태 · Goal 첫 문단
sh "${HANDOFF_PLUGIN_ROOT}/scripts/handoffs.sh" scan done   # 봉인된 노트, 같은 컬럼
```

`HANDOFF_PLUGIN_ROOT`는 실행 전에 사용하는 스킬 디렉토리의 두 단계 상위인 플러그인 설치 경로로 설정합니다. 훅 설정에서는 호스트가 제공하는 루트 변수를 사용합니다. 별도 인덱스 파일 없이 `scan`이 매번 디렉토리를 훑으므로, 목록이 실제 파일과 어긋날 일이 없습니다.
