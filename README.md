...
| 태그 | 기준(기본값) | 동작 |
|------|-------------|------|
| 🟢 | **35%** | "지금이 저장하기 좋은 시점"이라고 가볍게 언급 |
| 🟠 | **50%** | `/handoff:save` 실행을 분명하게 권장 |
| 🔴 | **75%** | 컨텍스트 부족을 경고하고 **"한 번 더 저장하시겠습니까?"** 라고 질문 |

- **자동으로 저장하지 않습니다.** 세 단계 모두 제안·질문일 뿐이고, 실제 저장은 사용자가 `/handoff:save`를 실행할 때만 일어납니다.
