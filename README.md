...
- **단일 파일 → 병렬 작업에서 깨짐.** 작업이 둘 이상이면 한 파일에 뒤섞입니다. 작업별 슬러그 파일로 분리해, "default 파일이 *단일 작업*과 *무명 작업*을 혼동시키는" 문제를 제거했습니다.
- **레포 루트 저장 → 커밋 오염.** `HANDOFF.md`가 git에 잡혀 리뷰·커밋 부담과 충돌을 일으킵니다. 홈 디렉토리로 옮겨 "개인 세션 노트"라는 성격을 분명히 했습니다.
- **cwd 기준 → 하위 디렉토리에서 길 잃음.** git 루트 기준 슬러그라 모노레포·하위 폴더에서 세션을 열어도 같은 handoff를 찾습니다.

그리고 원본에 없던 흐름을 더했습니다.

- **resume** — 노트를 읽고 Goal·Next Steps를 요약한 뒤 *실행 전에 멈춰* 사용자 확인을 받습니다(재개는 컨텍스트 로드이지, 계획에 대한 커밋이 아니므로). **What Didn't Work**의 실패 방법은 다시 시도하지 않습니다.
- **delete** — 끝난 작업을 정리해 노트가 무한히 쌓이지 않도록 라이프사이클을 닫습니다.
- **인덱스 제거** — 목록을 매번 `scan`으로 만들어, 인덱스와 실제 파일이 어긋나는 동기화 버그를 원천 차단했습니다.
